...
class HealthMonitor:
    ...
    def on_health_updated(self, result: dict):
        ...
        Args:
            result: Dict with connection status and health payload
        """
        if "error" in result:
        ...
        if status_changed:
            logger.info(
                ...
                f"ANN: {ann_status}, LLM: {llm_status}"
            )
        ...
        if status_changed:
            ...
            # Warn if models are loading
            if llm_status == "loading":
                logger.info("⏳ LLM model is warming up...")
            if ann_status == "loading":
                logger.info("⏳ ANN model is loading...")
